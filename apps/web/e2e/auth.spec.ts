...
test.describe('登录进壳层', () => {
  ...
  test('登录后主导航可进入仪表盘 / 消息 / 项目 / 日历 / 文件', async ({ page }) => {
    ...
    await page.getByRole('link', { name: /消息|Messenger|Messages/i }).first().click();
    ...
    await page.getByRole('link', { name: /日历|Calendar/i }).first().click();
    await expect(page).toHaveURL(/\/manage\/calendar/, { timeout: 15_000 });

    await page.getByRole('link', { name: /文件|Files?/i }).first().click();
