...
export const userBotKeys = {
  ...
  info: (id: number) => [...userBotKeys.all(), 'info', id] as const,
};
...
export function useUserBotInfo(id: number | undefined, enabled = true) {
  return useQuery({
...
export function useDeleteUserBot() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: { id: number; remark: string }) =>
